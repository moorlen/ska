...
public class UserPool {
    final F.ArchivedEventStream<UserPool.Event> applicationEvents = new F.ArchivedEventStream<UserPool.Event>(100);

    /**
     * For WebSocket, when a user join the room we return a continuous event stream
     * of ChatEvent
     */
    public F.EventStream<UserPool.Event> join(String user) {
        applicationEvents.publish(new Join(user));
        return applicationEvents.eventStream();
    }
    ...
    public void leave(String user) {
        applicationEvents.publish(new Leave(user));
    }
    ...
    public void say(String user, String text) {
        ...
        applicationEvents.publish(new Message(user, text));
    }
    ...
    public F.Promise<List<F.IndexedEvent<Event>>> nextMessages(long lastReceived) {
        return applicationEvents.nextEvents(lastReceived);
    }
     ...
     * each refresh
     */
    public List<UserPool.Event> archive() {
        return applicationEvents.archive();
    }
