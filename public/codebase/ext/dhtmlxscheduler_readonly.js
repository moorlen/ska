...
scheduler.attachEvent("onTemplatesReady", function () {
    ...
    var t = scheduler.config.lightbox.sections.slice(), a = scheduler.config.buttons_left.slice(), r = scheduler.config.buttons_right.slice();
    scheduler.attachEvent("onBeforeLightbox", function (e) {
        ...
        if (this.config.readonly_active) {
            for (var n = 0; n < i.length; n++)if ("recurring" == i[n].type) {
                this.config.readonly_active && i.splice(n, 1);

                break
            }
            for (var l = ["dhx_delete_btn", "dhx_save_btn"], d = [scheduler.config.buttons_left, scheduler.config.buttons_right], n = 0; n < l.length; n++)for (var s = l[n], o = 0; o < d.length; o++) {
                for (var _ = d[o], c = -1, u = 0; u < _.length; u++)if (_[u] == s) {
                    c = u;
                    break
                }
                -1 != c && _.splice(c, 1)
    ...
    scheduler._fill_lightbox = function () {
        ...
        alert(this.getEvent(e).readonly);
        ...
        if (this.config.readonly_active && (t.style.visibility = "",
            ...
            var r = this.getLightbox(), l = this._lightbox_r = r.cloneNode(!0);
